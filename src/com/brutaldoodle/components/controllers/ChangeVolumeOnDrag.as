...
package com.brutaldoodle.components.controllers
{
	...
	public class ChangeVolumeOnDrag extends TickedComponent
	{
		 ...
		 */
		private var _currentDraggedObject:IEntity;
		...
		/*
		 * Handle the drag for all possible mouse events (mouseup, mousedown, mouseover and mouseout)
		 * This also handles the action that needs to be done when the drag starts and stops
		 */
		private function mouseHandler (event:MouseEvent):void {
			switch (event.type) {
				case MouseEvent.MOUSE_DOWN: // Start dragging on mouse down
					this.registerForTicks = false;
					_currentDraggedObject = owner;
				...
				case MouseEvent.MOUSE_UP: // Stop dragging on mouse up
					...
					if (volumeType == "sfx" && _currentDraggedObject === owner) {
